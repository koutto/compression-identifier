...
def hexdump(src, length=16):
    ...
    for c in range(0, len(src), length):
        ...
        hex = ' '.join(["%02x" % ord(chr(x)) for x in chars])
        printable = ''.join(["%s" % ((ord(chr(x)) <= 127 and filters[ord(chr(x))]) or '.') for x in chars])
...
def print_zlib_header(data):
    ...
    #    |     DICTID    |
    #    +---+---+---+---+
    cm = ord(chr(data[0])) & 0x0F
    cinfo = (ord(chr(data[0])) & 0xF0) >> 4
    fcheck = ord(chr(data[1])) & 0x1F
    fdict = (ord(chr(data[1])) & 0x20) >> 5
    flevel = (ord(chr(data[1])) & 0xc0) >> 6
    flevel_str = ['Fastest algorithm', 'Fast algorithm', 'Default algorithm', 'Maximum compression']
    header_len = 6 if fdict else 2

    print_info('Header: {0}'.format(' '.join(["%02x" % ord(chr(x)) for x in data[0:header_len]])))
    print_info('   +--- CMF = {0:02x}'.format(ord(chr(data[0]))))
    print_info('   |  +--- CM (Compression Method)  = {0} {1}'.format(cm, '- Deflate' if (cm == 8) else ''))
    print_info('   |  +--- CINFO (Compression Info) = {0}'.format(cinfo))
    print_info('   +--- FLG = {0:02x}'.format(ord(chr(data[1]))))
    ...
    if fdict:
        print_info('   +--- DICTID = {0}'.format(' '.join(["%02x" % ord(chr(x)) for x in data[2:4]])))
...
def print_gzip_header(data):
    ...
    #    | CRC16 |
    #    +---+---+
    ftext = ord(chr(data[3])) & 0x01
    fhcrc = (ord(chr(data[3])) & 0x02) >> 1
    fextra = (ord(chr(data[3])) & 0x04) >> 2
    fname = (ord(chr(data[3])) & 0x08) >> 3
    fcomment = (ord(chr(data[3])) & 0x10) >> 4
    freserved = (ord(chr(data[3])) & 0xE0) >> 5
    fextra_field = None
    fname_field = None
    fcomment_field = None
    fhcrc_field = None
    ...
    header_len = 10
    if fextra:
        fextra_len = int("{0:x}{1:x}".format(ord(chr(data[10])), ord(chr(data[11]))), 16)
        header_len += 2 + fextra_len
        fextra_field = data[10:10 + 2 + int("{0:x}{1:x}".format(ord(data[10]), ord(chr(data[11]))), 16)]
    if fname:
        beg = header_len
        fname_len = 0
    ...
    print_info('Header: {0}'.format(' '.join(["%02x" % ord(chr(x)) for x in data[0:header_len]])))
    print_info('   +--- ID1 = {0:02x}'.format(ord(chr(data[0]))))
    print_info('   +--- ID2 = {0:02x}'.format(ord(chr(data[1]))))
    print_info('   +--- CM  = {0:02x}'.format(ord(chr(data[2])), '- Deflate' if (ord(data[2]) == 8) else ''))
    print_info('   +--- FLG = {0:02x}'.format(ord(chr(data[3]))))
    ...
    if fextra:
        print_info('   +--- Extra Field = {0}'.format(' '.join(["%02x" % ord(chr(x)) for x in fextra_field])))
    if fname:
        print_info('   +--- Original file Name = {0}'.format(' '.join(["%02x" % ord(chr(x)) for x in fname_field])))
    if fcomment:
        print_info('   +--- File comment = {0}'.format(' '.join(["%02x" % ord(chr(x)) for x in fcomment_field])))
    if fhcrc:
        print_info('   +--- CRC16 = {0}'.format(' '.join(["%02x" % ord(chr(x)) for x in fhcrc_field])))
