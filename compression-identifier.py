...
def print_gzip_header(data):
	...
	print_info('   |  +--- FTEXT 	= {0} {1}'.format(ftext, '- Probably ASCII text' if ftext else '- Probably not ASCII text'))
	print_info('   |  +--- FHCRC 	= {0} {1}'.format(fhcrc, '- CRC16 field present in header' if fhcrc else '- No CRC16 field in header'))
	print_info('   |  +--- FEXTRA 	= {0} {1}'.format(fextra, '- Extra field present in header' if fextra else '- No extra field in header'))
	print_info('   |  +--- FNAME 	= {0} {1}'.format(fname, '- Original filename in header' if fname else '- No original filename in header'))
	print_info('   |  +--- FCOMMENT\t= {0} {1}'.format(fcomment, '- File comment in header' if fcomment else '- No file comment in header'))
...
input_data.add_option('-f', '', help='File containing raw data', action='store', type='string', dest='file_raw', default=None)
input_data.add_option('', '--offset', help='Beginning offset for decompression', action='store', type='string', dest='offset', default=0)
...
ident.add_option('', '--scan', help='Scan input and search for compressed data', action='store_true', dest='auto_detect', default=False)

zlib_format = OptionGroup(parser, 'Zlib compressed format (RFC 1950)')
zlib_format.add_option('', '--zlib-c', help='Compress using Zlib', action='store_true', dest='compress_zlib', default=False)
zlib_format.add_option('', '--zlib-d', help='Decompress using Zlib', action='store_true', dest='decompress_zlib', default=False)

deflate_format = OptionGroup(parser, 'Deflate compressed format (RFC 1951)')
deflate_format.add_option('', '--deflate-c', help='Compress using Deflate', action='store_true', dest='compress_deflate', default=False)
deflate_format.add_option('', '--deflate-d', help='Decompress using Deflate', action='store_true', dest='decompress_deflate', default=False)
...
if options.offset:
	if options.offset.isdigit():
		...
		print_error('Specified offset ({0}) is not valid numeric value. Supported: decimal / hexa. Will use offset=0'.format(options.offset))
